...
long measureDistance() {
  ...
  //float sound_speed= (33100+(61*temp));
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if(values[0]=='<' && values[len-1]=='<'){
      ...
      //receive_data[1]=map(receive_data[1],-150,150,-170,170);
    }
  }
  ...
  switch(state){ //the cases are used to make the transition between formation states. After the transition and respective movimentation are done the slave boat can follow along in follow_free, meaning it'll stay in the same relative position
    case 0: //follow_free
      ...
      if((distance=measureDistance()) > receive_data[2]*1.1 && mtr_speed>=0){
        mtr_speed = constrain(mtr_speed*1.3,0,150);
      }
    ...
    case 1: //follow_side
      ...
      /*if(last_state==2 || first_change){
          ...
          rudder.write(30);
          analogWrite(pwmPin, mtr_speed);
          while((distance=measureDistance()) > 25){
            if(Serial.available()>0 ){
          ...
          rudder_angle=145;
          rudder.write(145);
          mtr_speed = constrain(mtr_speed*1.5,0,150);
          analogWrite(pwmPin, mtr_speed);
          while((distance=measureDistance()) > 30){
            delay(50);
            if(Serial.available()>0 ){
              Serial.readBytes(values, 20);
            }
      ...
      if(last_state==2 || first_change){ //transition between follow_behind and follow_side (known starting location)
        ...
        if((distance=measureDistance()) > 30 && first_check!=1){
          mtr_speed = constrain(mtr_speed*2,0,150);
          if(rudder_angle < 90){  
            rudder_angle+=10;
            rudder.write(rudder_angle);
          }
          ...
          //after that, starts going straight faster than the master boat until the measured distance is inferior to 30cm (sonar at 0º, to the side, to detect when side by side)
          if(sec_doin==0){
            first_check=1;
            sonar_angle=0;
            sonar.write(0);
          ...
          if((distance=measureDistance()) > 30){
            mtr_speed = constrain(mtr_speed*1.5,0,150);
            analogWrite(pwmPin, mtr_speed);
            if(rudder_angle > 90){  
              rudder_angle-=10;
    ...
    case 2: // follow_behind
      ...
      if(first_change){
        ...
        if((distance=measureDistance()) > 30/*receive_data[2]*/ && first_check!=1){
          analogWrite(pwmPin, mtr_speed*0.65);
        }else{
          ...
          if(sec_doin==0){
            ...
            rudder_angle=140;
            rudder.write(140);
            //distance=receive_data[2]+10;
            distance=40;
            delay(50);
          }
          if((distance=measureDistance()) > 35/*receive_data[2]*/ && sec_check!=1){
            mtr_speed = constrain(mtr_speed*2,0,150);
            analogWrite(pwmPin, mtr_speed);
            if(rudder_angle > 90){  
              rudder_angle-=10;
            ...
            if((distance=measureDistance()) > 35/*receive_data[2]*/){
              mtr_speed = constrain(mtr_speed*1.5,0,150);
              analogWrite(pwmPin, mtr_speed);
              if(rudder_angle < 90){
                rudder_angle+=10;
        ...
        if(first_check!=1 && sin((sonar_angle*PI/180))*(distance=measureDistance())<receive_data[2]*.6 ){ //turns slightly to the left while the vertical distance to the master boat is inferior to a certain threshold
          ...
          if((distance=measureDistance()) > receive_data[2]+5){
            mtr_speed = constrain(mtr_speed*2,0,150);
            analogWrite(pwmPin, mtr_speed);
            if(rudder_angle > 90){  
              rudder_angle-=2;
    ...
    case 3: // follow_circ
      ...
      if(first_check!=1 && cos((sonar_angle*PI/180))*(distance=measureDistance())<receive_data[2]*.6){ //turns slightly to the left while the horizontal distance to the master boat is inferior to a certain threshold
        sonar_angle--; sonar.write(sonar_angle); //sonar_angle increase (every iteration would be too fast)
        if(rudder_angle < 90){  
          rudder_angle+=2;
          rudder.write(rudder_angle);
        }
      ...
      //after that, starts going straight faster than the master boat until the measured distance is inferior to 10cm (sonar at 180º, to the side, to detect when side by side)
        ...
        if((distance=measureDistance()) > 30/*receive_data[2]*/ && sec_check!=1){
          mtr_speed = constrain(mtr_speed*1.5,0,150);
          if(rudder_angle > 90){  
            rudder_angle-=2;
            rudder.write(rudder_angle);
          ...
          if((distance=measureDistance()) > 35/*receive_data[2]*/ && tre_check!=1){
            if(rudder_angle > 90){  
              rudder_angle-=2;
              rudder.write(rudder_angle);
            }
            ...
            if((distance=measureDistance()) > 35/*receive_data[2]*/){
              ...
              first_change=first_check=first_doin=sec_doin=sec_check=tre_doin=tre_check=quad_doin=0;
              i=0;
              digitalWrite(directionPin, LOW);
            }
          }
